...
def extraire_page_depuis_image(
    path: str,
    page_number: int,
) -> dict[str, Any] | None:
    """
    Extrait toutes les transactions visibles sur une page bancaire.

    Qwen Vision est utilisé sans JSON Object Mode côté serveur.
    Le JSON est ensuite contrôlé par _groq_request() / _parser_json().

    Cela évite qu'une génération presque correcte soit entièrement
    rejetée par Groq avec json_validate_failed.
    """
    ...
    try:
        ...
        logger.warning(
            "Lecture page bancaire impossible : %s",
            error,
        )
        return None
    ...
    payload = {
        ...
        # Pas de raisonnement long pour de l'extraction OCR.
