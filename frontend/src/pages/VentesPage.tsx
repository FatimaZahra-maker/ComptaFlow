...
const VENTES_DATA = [
  { id: 1, entreprise: "STE MAROC PIECES AUTO SARL", type: "pdf", fichier: "fact_vente_015.pdf", client: "CLINIQUE ARGANA", cat: "Vente", ref: "FV2504-015", date: "24/04/2025", libelle: "Prestation de service", ht: "10 000.00", tva: "2 000.00 (20%)", ttc: "12 000.00", statut: "Saisi", validation: "Validé" },
  { id: 2, entreprise: "STE MAROC PIECES AUTO SARL", type: "pdf", fichier: "fact_vente_078.pdf", client: "FIDEU SARL", cat: "Vente", ref: "FV2505-078", date: "15/05/2025", libelle: "Vente marchandises", ht: "8 500.00", tva: "1 700.00 (20%)", ttc: "10 200.00", statut: "En cours", validation: "À vérifier" },
  { id: 3, entreprise: "STE MAROC PIECES AUTO SARL", type: "xlsx", fichier: "export_ventes_juin.xlsx", client: "FREE SAS", cat: "Vente", ref: "FV2506-120", date: "02/06/2025", libelle: "Abonnement annuel", ht: "15 300.00", tva: "3 060.00 (20%)", ttc: "18 360.00", statut: "Saisi", validation: "Validé" },
];
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FB] p-6 text-slate-800 font-sans">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6 mb-6">
        ...
        <div className="xl:col-span-3 bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col">
                  ...
                  <tr key={row.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2">
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-white ${row.type === 'pdf' ? 'bg-red-500' : 'bg-green-600'}`}>
                          {row.type.toUpperCase()}
                        </span>
                        <span className="text-blue-600 text-xs hover:underline cursor-pointer">{row.fichier}</span>
                      </div>
                    </td>
                    <td className="px-4 py-4 text-xs font-medium whitespace-normal leading-tight w-32">{row.client}</td>
                    <td className="px-4 py-4">
                      <span className="bg-green-50 text-green-600 border border-green-100 px-2 py-1 rounded text-xs">
                        {row.cat}
                      </span>
                    </td>
          ...
          <div className="border-t border-slate-200 px-4 py-3 flex items-center justify-between bg-slate-50 text-sm">
            <span className="text-slate-500 text-xs">Affichage de 1 à 3 sur 18 résultats</span>
        ...
        <div className="xl:col-span-1 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-2 bg-green-50/50 border border-green-100 rounded-lg cursor-pointer">
                <div className="w-8 h-8 bg-green-100 text-green-600 rounded flex items-center justify-center">🛍</div>
                <div>
                  <div className="text-sm font-medium text-green-700">Ventes</div>
                  <div className="text-xs text-green-500">Documents de vente</div>
                </div>
              </div>
              <div className="flex items-center gap-3 p-2 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors">
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-5">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                <span className="text-slate-500">Total Ventes (HT)</span>
                <span className="font-semibold text-slate-800">450 200.00 MAD</span>
              </div>
              <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                <span className="text-slate-500">Total TVA</span>
                <span className="font-semibold text-slate-800">90 040.00 MAD</span>
              </div>
              <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                <span className="text-slate-500">Total TTC</span>
                <span className="font-semibold text-slate-800">540 240.00 MAD</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        {/* Widget Achats (remplace Ventes ici pour le contexte) */}
        <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-4">
          <h3 className="font-medium text-blue-700 mb-4 text-sm flex items-center gap-2">
            🛒 Achats — Tableau des documents d'achat
          </h3>
          <div className="overflow-x-auto">
            <table className="w-full text-[10px] text-left">
              ...
              <tbody className="divide-y divide-slate-50">
                <tr>
                  <td className="py-2">STE ELECTROMAR</td>
                  <td className="py-2">FA2404-0158</td>
                  <td className="py-2 text-right font-medium">6 660.00</td>
                  <td className="py-2 text-center text-green-500">✓</td>
                </tr>
                <tr>
                  <td className="py-2">BATI SERVICES</td>
                  <td className="py-2">FS-2025-0789</td>
                  <td className="py-2 text-right font-medium">22 620.00</td>
                  <td className="py-2 text-center text-green-500">✓</td>
                </tr>
              </tbody>
            </table>
          </div>
          <a href="#" className="text-blue-600 text-xs mt-3 inline-block hover:underline">Voir tous les documents d'achat →</a>
        </div>
